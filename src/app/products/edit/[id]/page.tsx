...
// กำหนดประเภทข้อมูลสินค้าที่แก้ไข
...
export default function EditGroupPage({ params }: { params: { id: string } }) {
  ...
  const handleProductChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // จัดการกับค่าตัวเลข
    if (type === 'number') {
      updatedProducts[index][name as keyof Omit<EditableProductData, 'isEditing' | 'isDeleting'>] = 
        value === '' ? null : Number(value) as never;
    } else {
      updatedProducts[index][name as keyof Omit<EditableProductData, 'isEditing' | 'isDeleting'>] = value as never;
    }
